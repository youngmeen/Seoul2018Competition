...
public class FirstFragment extends Fragment implements OnMapReadyCallback {
    ...
    public String loadJSONFromAsset() {
        ...
        try {
            ...
            InputStream is = getActivity().getAssets().open("h1.json");
...
//액티비티가 처음 생성될 때 실행되는 함수
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        for (int i = 0; i < obj.length(); i++) {
            ...
            try {
                title = obj.getJSONObject(i).getString("instt_nm");
                x = obj.getJSONObject(i).getDouble("wgs84_la");
                y = obj.getJSONObject(i).getDouble("wgs84_lo");
                telephone = obj.getJSONObject(i).getString("reprsnt_tlphon");
            } catch (JSONException je) {
                je.printStackTrace();
            }
